...
class Layer(object):
    ...
    def _up_layer(self, bottom, shape,
        ...
        with tf.variable_scope(name):
            ...
            if shape is None:
                ...
                print('', type(in_shape), type(in_shape[0]))

                h = ((in_shape[1] - 1) * stride) + 1
                w = ((in_shape[2] - 1) * stride) + 1
                new_shape = [in_shape[0], h, w, num_classes]
            else:
                # num_classes_t = tf.constant(num_classes)
    ...
    def _up_layer_by_resize(self, bottom, shape):
        deconv = tf.image.resize_bilinear(bottom, [shape[1], shape[2]])
        _activation_summary(deconv)
        return deconv
    ...
    def get_conv_filter(self, name, shape=None, stddev=0.001):
        ...
        tf.add_to_collection('loss1', var)
    ...
    def get_fc_weight(self, name, init=None, shape=None):
        if self.data_dict and self.data_dict.get(name):
            init = tf.constant_initializer(value=self.data_dict[name][0],
                                           dtype=tf.float32)
            shape = self.data_dict[name][0].shape
    ...
    def get_fc_weight_reshape(self, name, shape, num_classes=None, init=None):
        print('Layer name: %s' % name)
